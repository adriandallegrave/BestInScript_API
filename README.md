...
| 1.13.0 | 2026-07-14 | 70b46ba | Default **panic key** changed from **Pause** to **`1`** for fresh setups: `OverlaySettings.StopAllHotkey` now defaults to `1` (still keyboard-only, still overridable from the header **Panic** control, still takes precedence over any script/preset on the same key). Additive/backwards-compatible — existing `overlay-settings.json` files load unchanged and keep whatever panic key they already saved; only new installs (and files with no `StopAllHotkey`) get the `1` default. No timing or synthetic-input change |
| 1.12.1 | 2026-07-13 | 1900d63 | Overlay settings panel **layout fix**: the dialog was a single tall, narrow column that overflowed a 1080p screen — the **Save** button fell below the viewport with no way to scroll to it. The dialog is now a flex column capped at `calc(100vh - 40px)` with a **scrollable body** and pinned header/footer (Save always reachable), **widened 440→780px**, and split into **two columns** — **Placement** controls left, **event-timer** config right — roughly halving its height. Collapses back to a single stacked column under a 720px viewport. UI-only (`index.html` + `_overlay-panel.html`); no data-file, timing, or synthetic-input change |
