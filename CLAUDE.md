...
Semantic versioning, tracked in the README's **Version history** table. Current: **1.15.0** (baseline **1.0.0** = commit `eb615c5`).
...
| Controllers | `Controllers/` — `ScriptsController.cs`, `PresetsController.cs`, `EngineController.cs`, `OverlayController.cs`, `ScreenController.cs`, `ProfilesController.cs`, `BuildCardsController.cs`, `SnapshotsController.cs` |
...
| Persistence | `Persistence/` — `JsonListFileStore.cs` (generic base), `ScriptRepository.cs`, `PresetRepository.cs`, `BuildCardRepository.cs`, `OverlaySettingsStore.cs`, `DataFilePathResolver.cs`, `ProfileManager.cs` (+ `IProfileScopedStore.cs`), `ConfigSnapshotService.cs` (rotating pre-write backups), `IScriptRepository.cs`, `IPresetRepository.cs`, `IBuildCardRepository.cs` |
...
| Web UI | `wwwroot/index.html` is the ONLY file served. Two self-contained modal blocks are **manually pasted** into it before `</body>`, each kept as a standalone copy that must be edited in **both** places: `wwwroot/_overlay-panel.html` (marker `BIS_OVERLAY_PANEL_MARKER`) and `wwwroot/_build-panel.html` (marker `BIS_BUILD_PANEL_MARKER`). A third block, the snapshots dialog (marker `BIS_SNAPSHOT_PANEL_MARKER`, `bis-sn-*` ids), lives in `index.html` **only — it has no standalone twin, deliberately** (BACKLOG 6.4 tracks paying off the paste debt; don't add to it). The root-level `overlay-settings-panel.html` is **stale legacy — ignore it**. App icon `wwwroot/app.ico` (tray + favicon + exe), located at runtime by `Services/WebAssetLocator.cs` |
| Config | `appsettings.json` (`BestInScript:DataDirectory` → `C:\temp`, `SnapshotCount` → 10) |
| Data (runtime) | `C:\temp\profiles\<name>\scripts.json` + `presets.json` + `build-cards.json` + `cards/*.png` + `.snapshots/*.json` (per profile), `C:\temp\profiles.json` (active pointer), `C:\temp\overlay-settings.json` + `C:\temp\.snapshots\` (global) |
...
| `HotkeyEngine`         | `IHostedService` façade: loads repos, wires hook → coordinator, delegates the public API. `ApplyGlobalHotkeys` resolves `OverlaySettings.StopAllHotkey` → `SetStopAllKey` and `BuildPanel.CycleHotkey` → `SetBuildCardKey` at startup and on every `OverlaySettingsStore.Changed`, keeping both live. `ReloadFromDisk` re-reads the active profile's files into a cleared coordinator — `SwitchProfile` minus the profile hop — for the snapshot-restore path. |
...
| `OverlaySettingsStore` | Holds + persists overlay settings (`overlay-settings.json`). Fires `Changed` event on save. Global — NOT profile-scoped. `Reload()` re-reads the file and republishes it through `Changed` (used after a snapshot restore); firing `Changed` is what re-arms the global hotkeys and restyles the live windows. |
| `ConfigSnapshotService` | Rotating pre-write backup of every JSON config file (BACKLOG 3.4). Each write calls `Capture(path)` first, which copies the file's CURRENT bytes into a `.snapshots/` directory beside it and prunes to the newest `BestInScript:SnapshotCount` (default 10; 0 disables). **Archives the pre-write bytes, never the new list** — the failure it protects against is `JsonListFileStore` treating a corrupt file as empty and overwriting it, so the copy has to be the original. Path-based and profile-unaware: the directory is derived from the file's own, so profile files land in `profiles/<name>/.snapshots/` and the global settings in `<base>/.snapshots/`. Capture failures are logged and swallowed — losing a backup must never break a save. `Restore` archives the live file first (so a restore is itself undoable) and hard-validates the client-supplied id as a bare snapshot name for that file. |
...
| `EventScheduleService` (`IHostedService`) | Diablo 4 event timers. One outbound HTTP GET of the `helltides.com/schedule` page at startup (the app's ONLY network call), cached in memory; `ScheduleResponseParser` pulls the schedule out of it. **Not `/api/schedule`:** that JSON API is behind a Cloudflare bot challenge (403 for any non-browser client, which silently emptied the overlay's clocks until v1.14.2), while the page server-renders the same data as a devalue-encoded Nuxt payload in its `__NUXT_DATA__` script. The parser still accepts plain API JSON, for a `ScheduleApiUrl` override. Failures log the HTTP status or "no schedule found"; `GetSnapshot(now)` returns the next world boss / helltide / legion via `EventScheduleCalculator`. Passive/informational; a failed fetch just hides the rows. Injected into `OverlayWindow` (via `OverlayHostedService`) and `OverlayController`. |
...
| `/api/snapshots`, `POST /api/snapshots/{target}/restore` | Config snapshots for the four `target`s `scripts` / `presets` / `build-cards` / `overlay-settings`. The snapshot id goes in the **body** (it is a file name), not the route. A restore refreshes in-process: scripts/presets → `HotkeyEngine.ReloadFromDisk` (running scripts stop), build cards → `BuildCardSignal.NotifyCardsChanged`, overlay settings → `OverlaySettingsStore.Reload`. |
...
- Bitmaps load through `Overlay/BuildCardImageLoader.cs` — split out because it needs no window/dispatcher and so *is* unit-testable (`BuildCardImageLoaderTests`), unlike the rest of the overlay. It decodes via a `MemoryStream` + `CacheOption.OnLoad` (no file lock, so the web UI can replace an image under it) with `DecodePixelWidth` set from `MaxWidth × Scale` **only when the source is wider** — downscale, never upscale — then freezes. The window caches the result per `(card id, decode width)`; the cache is cleared on `CardsChanged`.
  **Two WPF traps, both load-bearing (v1.14.0 shipped with the second and rendered every card title with no picture):** `CacheOption` must be assigned **before** `StreamSource` (WPF reads it when the source is set); and `BitmapCreateOptions.IgnoreImageCache` must **not** be set with a `StreamSource` — WPF's image cache is URI-keyed, a stream has no URI, so `EndInit()` throws `ArgumentNullException("key")`. The flag is pointless there anyway: a stream source bypasses the URI cache.
- A card whose image is missing or won't decode shows `⚠ image failed to load (…)` in the panel and logs the exception. It must never fail silently — a bare title is indistinguishable from a config mistake.
...
Per-profile: `profiles/<name>/scripts.json`, `presets.json`, `build-cards.json`, the `cards/` image directory, and `.snapshots/` (config history). Global: `overlay-settings.json` (at the base dir), `profiles.json` (the active-profile pointer), and a base-dir `.snapshots/` holding the overlay-settings history.
...
`.snapshots/` is deliberately **not** a declared `ProfileSubdirectory`, and three placement facts follow from that:
it must stay *inside* a profile dir (never a child of `profiles/`, which `ProfileManager.ListLocked` enumerates — it would be listed as a profile);
rename and delete carry/remove it for free; and `Create(copyFromCurrent: true)` does **not** copy it, so a new season profile starts with clean history instead of inheriting the old one's.
...
Config keys: `BestInScript:DataDirectory`, `BestInScript:DataFilePath`, `BestInScript:PresetsFilePath`, `BestInScript:OverlaySettingsPath`, `BestInScript:ScheduleApiUrl` (event-timer source, default `https://helltides.com/schedule`; the page payload and API JSON both parse), `BestInScript:EventsEnabled` (default true), `BestInScript:SnapshotCount` (config snapshots kept per file, default 10, clamped 0–100; 0 disables the feature). The default `appsettings.json` ships `DataDirectory: C:\temp` and `SnapshotCount: 10`.
